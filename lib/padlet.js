...
export function hasExistingBotFeedback(snapshot, postId, botDisplayName) {
  ...
  // postId 추출이 전반적으로 실패한 경우(모든 댓글이 postId="") 안전망:
  // 마커가 있는 댓글이 하나라도 있으면 중복으로 간주
  const allPostIdsEmpty = snapshot.comments.every((c) => !c.postId);
  if (allPostIdsEmpty && snapshot.comments.length > 0) {
    return snapshot.comments.some(isBotComment);
  }
