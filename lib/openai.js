...
export async function generateFeedback(config, context) {
  ...
  const instructions = [
    ...
    "글쓴이를 '교장선생님'이라고 호칭하라.",
    "글쓴이를 학생처럼 대하지 말고, 동료 교장선생님께 드리는 피드백처럼 쓰라.",
  ...
  const prompt = [
    ...
    "교사 게시글 원문:",
    `<<<${normalizedPostText}>>>`,
    "지시: 위 원문에서 실제로 보이는 표현에만 근거해, 참조 자료의 관련 개념과 연결하여 교장선생님의 교육적 전문성에 도움이 되는 동료형 피드백을 3문장 이하로 작성하라."
  ].join("\n");
...
function buildShortPostFeedback(postText) {
  ...
  return [
    ...
    `이 관점을 실제 학교 운영에서 구현하려면 어떤 장면에서 ${topic}이 우선되어야 하는지까지 드러나면 교육적 메시지가 더 선명해질 것 같습니다.`,
    `교장선생님께서 보시기에 ${topic}이 수업 목표, 활동, 평가 중 어디에서 가장 먼저 반영되어야 한다고 생각하시는지 한 가지 사례로 풀어 주실 수 있을까요?`
  ].join(" ");
}
