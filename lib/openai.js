...
export async function generateFeedback(config, context) {
  ...
  const instructions = [
    ...
    "응답 구조는 반드시 2문장으로만 구성하라.",
    "첫 문장: 글의 핵심 표현을 짚어 간결하게 공감하거나 의미를 인정하라. 과도한 칭찬은 피하라.",
    "둘째 문장: 글쓴이의 의견을 교장선생님의 네 가지 역할(학교경영·수업지원·교사역량·협력문화) 중 가장 관련 깊은 축과 연결해 열린 질문을 하라. 질문은 하나만, 구체적이고 실천 가능한 방향으로.",
    "글쓴이를 '교장선생님'이라고 호칭하라.",
  ...
  const prompt = [
    ...
    "게시글 원문:",
    `<<<${normalizedPostText}>>>`,
    "지시: 위 원문의 핵심 표현을 짚어 공감(1문장) + 교장선생님 역할(학교경영·수업지원·교사역량·협력문화)과 연결한 열린 질문(1문장), 총 2문장으로만 작성하라."
  ].join("\n");
...
function buildShortPostFeedback(postText) {
  ...
  return [
    ...
    `교장선생님께서 이 관점을 실제 학교에서 구현하실 때, 수업 지원·교사 역량·협력 문화 중 어느 축에서 가장 먼저 변화를 만들어 가실 계획인지 여쭤봐도 될까요?`
  ].join(" ");
}
